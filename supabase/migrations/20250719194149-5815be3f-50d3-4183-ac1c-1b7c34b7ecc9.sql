...
CREATE INDEX IF NOT EXISTS idx_service_credentials_user_id_active ON public.service_credentials(user_id) WHERE expires_at IS NULL OR expires_at > now();
CREATE INDEX IF NOT EXISTS idx_migration_projects_user_id_status ON public.migration_projects(user_id, status);
CREATE INDEX IF NOT EXISTS idx_demo_sessions_expires_at ON public.demo_sessions(expires_at) WHERE expires_at > now();